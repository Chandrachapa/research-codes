...
alpha = age{time_index,robo_index}(:)+ age_increment >  lifetime{robo_index}; %conditional sensor failure
...
if round(sum_replacements) > 0 %if number of replacements is positive new sensor replacement
    lifetime{robo_index}(decision_sensor == 1) = ...
        new{robo_index}(1:round(sum_replacements));
    age{time_index,robo_index}(round(decision_sensor) == 1) = 0; %age of replaced sensors made zero
    new{robo_index}(1:round(sum_replacements)) = [];%places of new sensors which were used are emptied
end
