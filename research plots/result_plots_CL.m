...
% figure(4)
% hold all
% plot(mean(data_count_CL_rng,1),mean(mean_LL_CL_rng,1),'bd-','LineWidth',1)
% xlabel('data count')
% ylabel('LL')
% grid on
% grid minor
...
% [mean_avg_replacement_costCL_rng,sortIdx]=sort(mean(mean_avg_replacement_costCL_rng,1),'descend');
% mean_avg_comm_costCL_rng = mean_avg_comm_costCL_rng(sortIdx);
% plot(mean_avg_replacement_costCL_rng,mean_avg_comm_costCL_rng,'bd-','LineWidth',1)
% xlabel('avg replacement cost')
% ylabel('avg comm cost')
% grid on
% grid minor
...
% figure(6)
% hold all
% y3 = mean(mean_varianceCL_rng,1) - variance_threshold;%ccdf for 30 robots 
% [f,x3]=ecdf(y3);
% myccdf = 1-f;
% plot(x3,myccdf,'bd-','LineWidth',1)
% hold on 
...
x3 = 1:15;
y3 = mean(mean_outageCL_rng,1);
plot(x3,mean(mean_outageCL_rng,1),'g-','LineWidth',1)
p = polyfit(x3,y3,6);
f = polyval(p,x3);
hold on
plot(x3,f,'g-','LineWidth',1)
...
figure(11)
hold on
plot(1:15 ,(mean(mean_varianceCL_rng,1) - variance_threshold),'g-','LineWidth',1)
grid on;
grid minor;
xlabel('robot count')
ylabel('error')