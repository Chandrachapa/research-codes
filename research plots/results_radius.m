...
mean_outageFL(SNR_index) =mean(mean(outage,2),1);%average outage probability of the system per time unit
% save(sprintf('mean_outageFL_%d', total_no_robots), 'mean_outageFL');

mean_varianceFL(SNR_index) = mean(mean(variance,2),1);%average variance of the system per time unit
% save(sprintf('mean_varianceFL_%d', total_no_robots), 'mean_varianceFL');

data_count_FL(SNR_index) = sum(sum(data_count,2)); 
% save(sprintf('data_countFL_%d',total_no_robots), 'data_count_FL');

mean_optimal_costFL(SNR_index) =  mean(mean(optimal_cost,2),1);
% save(sprintf('optimal_costFL_%d',total_no_robots), 'mean_optimal_costFL');

mean_avg_replacement_costFL(SNR_index) = mean(mean(avg_replacement_cost,2),1);
% save(sprintf('avg_replacement_costFL_%d',total_no_robots), 'mean_avg_replacement_costFL');

mean_avg_comm_costFL(SNR_index) =  mean(mean(avg_comm_cost,2),1);
% save(sprintf('avg_comm_costFL_%d',total_no_robots), 'mean_avg_comm_costFL');

mean_LL_FL(SNR_index) = mean(mean(LL,2),1);
