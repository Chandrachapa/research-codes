...
plot(1:target_robots ,mean(mean_outageestH_rng,1),'k--','LineWidth',1)
xi = linspace(min(x3), max(x3), 150);                     % Evenly-Spaced Interpolation Vector
yi = interp1(x3, y3, xi, 'spline', 'extrap');
hold on
plot(xi,yi,'-dr','LineWidth',1)
...
hold on
x3 = 1:target_robots;
y3 = (mean(mean_varianceestH_rng,1) - variance_threshold);
plot(1:target_robots ,(mean(mean_varianceestH_rng,1) - variance_threshold),'k-','LineWidth',1)
xi = linspace(min(x3), max(x3), 150);                     % Evenly-Spaced Interpolation Vector
yi = interp1(x3, y3, xi, 'spline', 'extrap');
hold on
plot(xi,yi,'-dr','LineWidth',1)
...
% % figure(4)
% % hold all
% % plot(data_count,abs(mean_LL_estH),'g+:','LineWidth',2)
% % xlabel('data count')
% % 
% % figure(5)
% % hold all
